...
public class ApiTests {
	...
	//i. Successful APIrequest and response. 
	//ii. Validation of response status codes.
	//iii.Validation of response data (e.g., JSON schema validation).
	...
	public void test1() {
		...
		response.then().assertThat().statusCode(200).body(matchesJsonSchemaInClasspath("schema.json"));
	...
	//Handling of error responses and edge cases.
	...
	public void test3() {
		...
		Response response = given().header("Content-Type", "application/json").body(requestBody).when()
				.post("/students").then().statusCode(201) // Created
				.body("name", equalTo("Rahul Singh")).body("age", equalTo(16)).body("grade", equalTo("10th"))
				.body("subjects", hasSize(3)).body("subjects", hasItems("Computer Science", "Philosophy", "Art"))
				// JSON schema validation (studentSchema.json in src/test/resources)
				.body(matchesJsonSchemaInClasspath("schemaput.json")).extract().response();
	...
	// Updating existing user details using put method
	...
	public void test4() {
		...
		// Send PUT request
		Response response = given().header("Content-Type", "application/json").body(requestBody).when()
				.put("/students/2").then().statusCode(200) // success
				.body("id", equalTo("2")).body("name", equalTo("Jane Smith")).body("age", equalTo(17))
				.body("grade", equalTo("PostDoc")).body("subjects", hasItems("Biology", "Chemistry", "History"))
				.body("subjects", hasSize(3)).extract().response();
	...
	// Deleting a user and validating its record 
	...
	public void test5() {
		...
		Response postResponse = given().header("Content-Type", "application/json").body(requestBody).when()
				.post("/students").then().statusCode(201) // Created
				.body("name", equalTo("Rahul Singh")).body("age", equalTo(16)).body("grade", equalTo("10th"))
				.body("subjects", hasItem("Astronomy")).extract().response();
		...
		given().when().delete("/students/" + studentId).then().statusCode(200); // JSON server returns 200 on delete

		System.out.println("Deleted student 'Rahul Singh' with ID: " + studentId);

		// Step 4: Verify deletion
		given().when().get("/students/" + studentId).then().statusCode(404); // Student no longer exists
